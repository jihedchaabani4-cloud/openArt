...
export function ImageInfoSidebar({ item, group, onClose }) {
    ...
    const metadata = item.mediaMetadata?.requestData || {}
    const settings = metadata.settings || {}

    const prompt = metadata.promptInputs?.[0]?.textInput || item.params?.prompt || item.prompt || group?.params?.prompt || "No prompt available"
    const model = metadata.modelName || metadata.modelId || item.model || group?.model || "Stable Diffusion"
    const quality = settings.quality || item.params?.quality || group?.params?.quality || "1K"
    const ratio = settings.ratio || item.params?.ratio || group?.params?.ratio || "1:1"
    
    const mediaObj = item.image || item.video;
    const width = mediaObj?.dimensions?.width || item.params?.width || group?.params?.params?.width || group?.params?.width || ""
    const height = mediaObj?.dimensions?.height || item.params?.height || group?.params?.params?.height || group?.params?.height || ""
    const sizeStr = settings.size || item.params?.size || group?.params?.size || group?.params?.params?.size;
    ...
    const isLiked = item.is_Like || item.is_liked || false
    const url = item.url || mediaObj?.url || item.file_url || item.asset?.file_url
                        ...
                        {TABS.map((tab) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        "flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg text-[11px] font-medium transition-all duration-200 whitespace-nowrap",
                                        isActive
