    ...
    library.forEach(item => {
      if (item.items && Array.isArray(item.items)) {
        item.items.forEach(i => {
          if (i.status !== 'deleted' && i.asset) {
            flatItems.push({
              ...i,
              project_id: item.project_id,
              group_id: item.id,
              is_video: i.asset.asset_type === 'video',
              url: i.asset.url || i.asset.file_url,
              likes_count: i.likes_count || 0,
              is_Like: i.is_liked || i.is_Like, // Map likes consistently
              asset_id: i.asset_id || i.asset.id,
            });
          }
        });
      } else {
        flatItems.push({
          ...item,
          url: item.url || item.file_url || item.asset?.url || item.asset?.file_url,
          is_video: item.is_video || item.asset?.asset_type === 'video',
          asset_id: item.asset_id || item.asset?.id || item.id,
        });
      }
    });
          ...
          {visible.length === 0 && !loading ? (
            ...
            <div className={cn("grid gap-2", colClass)}>
              {visible.map((item, idx) => {
                const url = item.url || item.file_url || item.asset?.url || item.asset?.file_url;
                const isVideo = item.is_video;
                return (
                  <div
                    key={`${item.group_id ?? 'asset'}-${item.id ?? idx}-${idx}`}
