...
export function GenerationsPromptBar({ onGenerateAction }) {
  const { 
    projectId, activeSessionId, activeShotId, addShot, updateShot, activeSceneId, getActiveShot,
    scenes, addScene, fetchAssets, fetchGenerations, activeModel,
  ...
  const [libraryTab, setLibraryTab] = useState("all"); // "workspace", "all", "liked"
  ...
  const handleSelectFromLibrary = (assetObj) => {
    if (referenceImages.length >= 4) {
      alert("You can only add up to 4 reference images.");
      return;
    }
    setReferenceImages(prev => [...prev, assetObj]);
    setShowAssetLibrary(false);
  };

  // Reference Image States
  const [referenceImages, setReferenceImages] = useState([]); // array of { url, asset_id, file }
    ...
    filesToUpload.forEach(file => {
      ...
      reader.onloadend = () => {
        setReferenceImages(prev => {
          ...
          return [...prev, { url: reader.result, file: reader.result }];
        });
      };
  ...
  const handleGenerate = async (e) => {
    ...
    setGenerating(true);
    try {
      // Format references for the backend
      const formattedReferences = referenceImages.map(ref => ({
        role: 'source',
        url: ref.asset_id ? ref.url : undefined,
      ...
      const payload = {
        ...
        model_name: activeModel?.id || "nanobana_pro", // Force current selected model here
        image_base64: referenceImages[0]?.file || null,
        images_base64: referenceImages.map(r => r.file).filter(Boolean),
        strength: strength,
        edit_type: referenceImages.length > 0 ? 'img2img' : 'txt2img',
        references: formattedReferences
      };

      console.log("🚀 [Frontend Req] Sending Generation Payload:", JSON.stringify({ ...payload, image_base64: payload.image_base64 ? "[BASE64_OMITTED]" : null, images_base64: "[ARRAY_OMITTED]" }, null, 2));

      // Call the backend directly - no forced scene/shot creation
      const { api } = await import("@/lib/api");
      const res = await api.post(`/generations/generate`, payload);
  ...
  const handleDrop = (e) => {
    ...
    if (imageUrl) {
      handleSelectFromLibrary({ url: imageUrl, asset_id: assetId });
    }
  };

  // Shared pill-style dropdown using Shadcn DropdownMenu
  const PillDropdown = ({ value, options, onChange, renderOption, renderValue }) => (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        side="top"
        align="center"
        className="bg-[#1a1a1a] border border-white/10 rounded-xl p-1.5 min-w-[120px] shadow-[0_8px_32px_rgba(0,0,0,0.6)]"
      >
        {options.map((opt) => (
          <DropdownMenuItem
            key={opt}
            ...
            className={cn(
              "flex items-center gap-2 rounded-lg px-3 py-2 text-[13px] cursor-pointer transition-colors",
              opt === value
      ...
      <div style={{ width: "100%", maxWidth: "780px", position: "relative" }}>
        ...
        >
          <form
            ...
            onDrop={handleDrop}
            style={{
              background: isOver ? "rgba(255,255,255,0.08)" : "rgba(13,14,15,0.72)",
              borderRadius: "18px",
              padding: "16px 18px",
                        ...
                        return (
                          <div 
                            ...
                            onDragStart={(e) => {
                              ...
                              const assetId = item.id || item.asset?.id;
                              e.dataTransfer.setData("imageUrl", url);
                              if (assetId) e.dataTransfer.setData("assetId", assetId);
                              e.dataTransfer.effectAllowed = "copy";
                            }}
                            onClick={() => {
                              const url = item.file_url || item.asset?.file_url;
                              const assetId = item.id || item.asset?.id;
                              handleSelectFromLibrary({ url, asset_id: assetId });
                            }}
                            className={cn(
