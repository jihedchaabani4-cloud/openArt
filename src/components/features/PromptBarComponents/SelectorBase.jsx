...
const RatioIcon = ({ ratio, isSelected }) => {
  ...
  if (w >= h) {
    rw = baseW - padding * 2;
    rh = Math.round((h / w) * rw);
  } else {
    rh = baseH - padding * 2;
    rw = Math.round((w / h) * rh);
  }
    ...
    <div className={cn(
      ...
      <svg width={rw} height={rh} viewBox={`0 0 ${rw} ${rh}`} fill="none">
         <rect 
           ...
           stroke="currentColor" 
           strokeWidth="1.5" 
           className={isSelected ? "text-blue-500" : "text-white/80"}
         />
      </svg>
    </div>
  );
...
export function SelectorBase({
  ...
  const filterItems = (itemList) => (itemList || []).filter(item => {
    const label = (typeof item === "object" && item !== null) ? item.label : String(item)
  ...
  return (
    <>
      <style>{`
        ...
        .selector-scroll::-webkit-scrollbar { width: 5px; }
                                    ...
                                    {(item.type === 'ratio' || (typeof itemLabel === 'string' && itemLabel.includes(':') && listClassName?.includes('grid'))) ? (
                                      <div className="flex flex-col items-center gap-2 group w-full">
                                        ...
                                        <span className={cn(
                                          ...
                                          isSelected ? "text-blue-500" : "text-[#e3e4e7] group-hover:text-white"
                                        )}>
                                          {itemLabel}
                                        </span>
                                      </div>
