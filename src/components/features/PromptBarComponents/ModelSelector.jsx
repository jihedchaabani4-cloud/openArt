...
const getFamilyId = (key) => {
  ...
  if (k.includes("z-image"))  return "zimage";
...
export function ModelSelector({ type = "video", defaultId, onChange, dynamicModels, loading }) {
  ...
  const mergedModels = React.useMemo(() => {
    if (!dynamicModels || dynamicModels.length === 0) return ALL_MODELS;

    const mappedDynamic = dynamicModels.map(apiModel => {
      ...
      return {
        ...
        maxReferences: apiModel.references?.max ?? 0
      };
    });

    // Merge: Use dynamic models for images, keep static models for non-images
    const staticNonImage = ALL_MODELS.filter(m => m.type !== "image");
    return [...mappedDynamic, ...staticNonImage];
  }, [dynamicModels]);
