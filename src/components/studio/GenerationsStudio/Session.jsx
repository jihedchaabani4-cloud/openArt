...
import { getGridClass, getItemMetadata } from "@/utils/generationUtils";
...
function SidePanel({ group, prompt, modelLabel, onDelete, onRetry, onGenerateMore, onDownload }) {
    ...
    return (
        <div
            ...
            style={{ width: isOpen ? 200 : 32, transition: "width 0.22s cubic-bezier(.4,0,.2,1)" }}
            onMouseEnter={() => setHovered(true)}
            onMouseLeave={() => setHovered(false)}
        >
            {/* ─── Collapsed — 3 dots indicator ─── */}
            <AnimatePresence>
                {!isOpen && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        ...
                        className="flex flex-col items-center gap-1 pt-1 cursor-pointer"
                    >
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute top-0 left-0 w-[200px] flex flex-col gap-3 pt-0.5"
                        style={{ zIndex: 10 }}
                    >
                        {/* Buttons */}
...
// ─── Session ───
export function Session({ group, studioMode, gridSize = 'lg' }) {
    const {
